...
# ========================================================
#  HEALTH-CHECK HTTP SERVER (Render port talabi + UptimeRobot uchun)
# ========================================================
...
class _HealthHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        pass  # konsolni chalkashtirmaslik uchun so'rov loglari o'chirilgan
...
async def get_due_withdraw_reminders():
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            ...
            "WHERE status='approved' AND reminder_sent=0 AND approved_at IS NOT NULL "
...
async def check_one_channel(context, channel_id, user_id):
    ...
    if result is None:
        # Tekshiruv amalga oshmadi (masalan, bot kanalda admin emas).
        # Xavfsizlik uchun foydalanuvchi OBUNA BO'LMAGAN deb hisoblanadi,
        # aks holda majburiy obuna talabi chetlab o'tiladi.
...
def main_keyboard(user_id):
    ...
    # SUPER_ADMIN va bazaga qo'shilgan barcha adminlar Admin Panel tugmasini ko'radi.
    # _admins_cache get_admins() orqali doimiy yangilanadi (birinchi so'rovda to'ldiriladi).
    if user_id == SUPER_ADMIN or (_admins_cache is not None and user_id in _admins_cache):
...
async def handle_support_state(update, context, user_id, text):
    ...
    admins = await get_admins()
    # parse_mode ishlatilmaydi — foydalanuvchi matnida Markdown belgilari
    # (_, *, ` va h.k.) bo'lsa ham xabar adminga muvaffaqiyatli yetib borishi uchun.
    admin_text = (
        f"☎️ Yangi murojaat!\n{DIVIDER}\n"
...
async def withdraw_admin_decision_callback(update, context):
    try:
        ...
        if action == "wdok":
            ...
            await api_call(lambda: context.bot.send_message(
                chat_id=target,
                text=f"✅ *Pul tasdiqlandi!*\n💰 *{amount:,.0f} so'm*\n⏳ Taxminan *1 soat* ichida hisobingizga tushadi.",
    ...
    try:
        ...
        if data == "leave_yes_3":
            ...
            for aid in admins_list:
                await api_call(lambda a=aid: context.bot.send_message(chat_id=a, text=f"⚠️ {query.from_user.full_name} chiqdi!"), action_desc="lvn")
            return
...
async def withdrawal_reminder_loop(app):
    """Tasdiqlangan pul yechish so'rovlaridan 1 soat o'tgach,
    foydalanuvchidan pul tushganini so'rab eslatma yuboradi.
    DB'da saqlanadigan holat orqali ishlaydi, shuning uchun
    bot qayta ishga tushsa ham eslatmalar yo'qolmaydi."""
    while True:
        try:
