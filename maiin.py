...
TOKEN = os.environ.get("BOT_TOKEN", "8856340901:AAEvMUbnqiRXQj--KyCgshF5syx0pWssUe4")
SUPER_ADMIN = 8057184376

DB_PATH = os.environ.get("DB_PATH", "bot_data.db")
WITHDRAW_REMINDER_SECONDS = 3600
BOT_USERNAME = None
SUB_CACHE_TTL = 300
...
class _HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot ishlamoqda \xe2\x9c\x85")
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute('CREATE TABLE IF NOT EXISTS withdrawals (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, amount REAL, card_type TEXT, card_number TEXT, status TEXT DEFAULT "pending", created_at TEXT DEFAULT CURRENT_TIMESTAMP, approved_at TEXT, reminder_sent INTEGER DEFAULT 0)')
        for col_sql in (
            'ALTER TABLE withdrawals ADD COLUMN approved_at TEXT',
            'ALTER TABLE withdrawals ADD COLUMN reminder_sent INTEGER DEFAULT 0',
        ):
            try:
        ...
        # Agar adminlar umuman yo'q bo'lsa, xavfsizlik uchun faqat default adminni qo'shamiz
...
async def set_withdrawal_status(wid, status, set_approved_time=False):
    async with aiosqlite.connect(DB_PATH) as db:
        if set_approved_time:
            await db.execute('UPDATE withdrawals SET status=?, approved_at=CURRENT_TIMESTAMP WHERE id=?', (status, wid))
        else:
            await db.execute('UPDATE withdrawals SET status=? WHERE id=?', (status, wid))
        await db.commit()


async def get_due_withdraw_reminders():
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "SELECT id, user_id, amount FROM withdrawals "
            "WHERE status='paid' AND reminder_sent=0 AND approved_at IS NOT NULL "
            "AND datetime(approved_at, '+' || ? || ' seconds') <= datetime('now')",
            (WITHDRAW_REMINDER_SECONDS,))
        return await cursor.fetchall()


async def mark_withdraw_reminder_sent(wid):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute('UPDATE withdrawals SET reminder_sent=1 WHERE id=?', (wid,))
        await db.commit()
...
async def check_one_channel(context, channel_id, user_id):
    ...
    if result is None:
        logger.warning("Kanal tekshiruvi muvaffaqiyatsiz: %s (bot kanalda admin emasmi?)", channel_id)
        return False
...
def main_keyboard(user_id):
    ...
    if _admins_cache is not None and user_id in _admins_cache:
...
def withdraw_reminder_keyboard():
    return InlineKeyboardMarkup([[InlineKeyboardButton("☎️ Murojaat", callback_data="start_support")]])
...
def admin_withdraw_paid_keyboard(wid):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ To'lov qilindi", callback_data=f"wdpaid:{wid}")]
    ])
...
async def start(update, context):
    try:
        ...
        if _bot_maintenance and user_id not in await get_admins():
            ...
            if referrer_id and referrer_id != user_id:
                ...
                if current_ref is None:
                    ...
                    if referrer:
                        ...
                        await api_call(lambda: context.bot.send_message(
                            chat_id=referrer_id,
                            text=f"🎉 *Sizning havolangiz orqali do'stingiz qayta kirdi!*\n💰 *+{ref_price:,.0f} so'm*",
                            parse_mode="Markdown"))
...
async def handle_payment_channel(update, context, user_id):
    ...
    await api_call(lambda: update.message.reply_text(
        text,
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📢 O'tish", url=f"https://t.me/{cid.replace('@', '')}")]])))
...
async def handle_support_state(update, context, user_id, text):
    ...
    admin_text = (
        f"☎️ Yangi murojaat!\n{DIVIDER}\n"
        f"👤 {user_info.full_name} {uname}\n"
        f"🆔 {user_info.id}\n{DIVIDER}\n"
        f"💬 {text}\n\n📋 ID: {sid}"
    )
    sent_ok = False
    for aid in admins:
        res = await api_call(lambda a=aid: context.bot.send_message(
            chat_id=a, text=admin_text, reply_markup=support_answer_keyboard(sid)), action_desc=f"sup_a:{aid}")
        if res:
            sent_ok = True
    if sent_ok:
        await api_call(lambda: update.message.reply_text(
        ...
        await api_call(lambda: update.message.reply_text(
            "⚠️ Xabar qabul qilindi, lekin adminga yetkazishda muammo bo'ldi. Birozdan so'ng qayta urinib ko'ring.",
            reply_markup=main_keyboard(user_id)), action_desc="sup_fail")
    return True
...
async def start_support_callback(update, context):
    try:
        query = update.callback_query
        await api_call(lambda: query.answer(), action_desc="ss_a")
        context.user_data["state"] = "support_message"
        await api_call(lambda: query.message.reply_text(
            f"☎️ *Murojaat*\n{DIVIDER}\nXabaringizni yozing:",
            parse_mode="Markdown", reply_markup=cancel_keyboard()), action_desc="ss_p")
    except Exception:
        logger.exception("start_support xato")
...
async def handle_support_reply_state(update, context, user_id, text):
    ...
    res = await api_call(lambda: context.bot.send_message(
        chat_id=target,
        text=f"💬 Admin javobi:\n{DIVIDER}\n{text}",
        reply_markup=main_keyboard(target)), action_desc="sr_s")
    if res:
        await api_call(lambda: update.message.reply_text("✅ Yuborildi!", reply_markup=main_keyboard(user_id)), action_desc="sr_ok")
    else:
        await api_call(lambda: update.message.reply_text("⚠️ Foydalanuvchiga yetkazib bo'lmadi (botni bloklagan bo'lishi mumkin).", reply_markup=main_keyboard(user_id)), action_desc="sr_fail")
    return True
...
async def handle_withdraw_state(update, context, user_id, text):
    ...
    context.user_data["withdraw_card_number"] = card_number
    context.user_data["state"] = "withdraw_amount"
    await api_call(lambda: update.message.reply_text(
        f"💰 *Qancha yechmoqchisiz?*\n{DIVIDER}\n💳 Balansingiz: *{balance:,.0f} so'm*\n📉 Minimal: *{MIN_WITHDRAW:,.0f} so'm*\n\nMiqdorni raqamda kiriting:",
        parse_mode=ParseMode.MARKDOWN, reply_markup=cancel_keyboard()), action_desc="wa_p")
    return True


async def handle_withdraw_amount_state(update, context, user_id, text):
    if context.user_data.get("state") != "withdraw_amount":
        return False
    if text == CANCEL_TEXT:
        context.user_data["state"] = None
        context.user_data.pop("withdraw_card_type", None)
        context.user_data.pop("withdraw_card_number", None)
        await api_call(lambda: update.message.reply_text("🚫 Bekor qilindi.", reply_markup=main_keyboard(user_id)), action_desc="wa_c")
        return True
    try:
        amount = float(text.strip().replace(" ", "").replace(",", "").replace("so'm", "").replace("сум", ""))
    except:
        await api_call(lambda: update.message.reply_text("❌ Miqdorni raqamda kiriting.", parse_mode=ParseMode.MARKDOWN), action_desc="wa_e")
        return True
    if amount <= 0:
        await api_call(lambda: update.message.reply_text("❌ Noto'g'ri miqdor.", parse_mode=ParseMode.MARKDOWN), action_desc="wa_z")
        return True
    user = await get_user(user_id)
    balance = user[1] if user else 0.0
    if amount < MIN_WITHDRAW:
        await api_call(lambda: update.message.reply_text(
            f"🚫 Minimal miqdor: *{MIN_WITHDRAW:,.0f} so'm*", parse_mode=ParseMode.MARKDOWN), action_desc="wa_min")
        return True
    if amount > balance:
        await api_call(lambda: update.message.reply_text(
            f"🚫 Balansingizdan ko'p. Balans: *{balance:,.0f} so'm*", parse_mode=ParseMode.MARKDOWN), action_desc="wa_max")
        return True
    card_type = context.user_data.get("withdraw_card_type", "Boshqa")
    card_number = context.user_data.get("withdraw_card_number", "")
    await add_balance(user_id, -amount)
    cache_clear_sub(user_id)
    ...
    context.user_data.pop("withdraw_card_number", None)
    await api_call(lambda: update.message.reply_text(
        f"✅ *So'rov qabul qilindi!*\n{DIVIDER}\n💰 *{amount:,.0f} so'm*\n💳 *{card_type}* `{card_number}`\n\n⏳ Admin tekshirgach tushadi.",
        parse_mode=ParseMode.MARKDOWN, reply_markup=main_keyboard(user_id)), action_desc="ws_ok")
    requester = update.effective_user
    uname = f"@{requester.username}" if requester.username else "yo'q"
    admins = await get_admins()
    admin_text = (
        f"💸 Pul yechish\n{DIVIDER}\n"
        f"👤 {requester.full_name} {uname}\n"
        f"🆔 {requester.id}\n"
        f"💰 {amount:,.0f} so'm\n"
        f"💳 {card_type} {card_number}\n"
        f"📋 {wid}"
    )
    sent_ok = False
    for aid in admins:
        res = await api_call(lambda a=aid: context.bot.send_message(
            chat_id=a, text=admin_text, reply_markup=admin_withdraw_keyboard(wid)), action_desc=f"ws_a:{aid}")
        if res:
            sent_ok = True
    if not sent_ok:
        logger.error("Pul yechish so'rovi #%s hech bir adminga yetkazilmadi!", wid)
    return True
...
async def withdraw_admin_decision_callback(update, context):
    try:
        ...
        if action == "wdok":
            await set_withdrawal_status(wid, "approved", set_approved_time=True)
            try:
                new_text = (query.message.text_markdown or query.message.text or "") + "\n\n⏳ Tasdiqlandi, to'lov kutilmoqda..."
            except:
                new_text = (query.message.text or "") + "\n\n⏳ Tasdiqlandi, to'lov kutilmoqda..."
            await api_call(lambda: query.message.edit_text(new_text, parse_mode=ParseMode.MARKDOWN, reply_markup=admin_withdraw_paid_keyboard(wid)), action_desc="wd_oe")
            
        elif action == "wdpaid":
            await set_withdrawal_status(wid, "paid")
            try:
                new_text = (query.message.text_markdown or query.message.text or "").replace("⏳ Tasdiqlandi, to'lov kutilmoqda...", "✅ To'lov qilindi")
            except:
                new_text = (query.message.text or "") + "\n\n✅ To'lov qilindi"
            ...
            await api_call(lambda: context.bot.send_message(
                ...
                parse_mode=ParseMode.MARKDOWN), action_desc="wd_pn")
            ...
            try:
                ...
                new_text = (query.message.text or "") + "\n\n❌ Rad etildi"
            await api_call(lambda: query.message.edit_text(new_text, parse_mode=ParseMode.MARKDOWN), action_desc="wd_ne")
            await api_call(lambda: context.bot.send_message(
                chat_id=target,
                text=(f"❌ *Pul yechish rad etildi*\n{DIVIDER}\n"
                      f"💰 *{amount:,.0f} so'm* hisobingizga qaytarildi.\n"
                      f"⚠️ Sabab: texnik muammo yoki shubhali to'lov."),
                parse_mode=ParseMode.MARKDOWN), action_desc="wd_un")
    except Exception:
...
async def admin_panel_callback(update, context):
    try:
        ...
        if data == "leave_yes_2":
            ...
            return
        if data == "leave_yes_3":
            context.user_data["state"] = "transfer_ownership"
            await api_call(lambda: query.message.edit_text("Bot kimga qoladi? (Yangi admin ID sini kiriting):", parse_mode=ParseMode.MARKDOWN), action_desc="lv3_transfer")
            await api_call(lambda: query.message.chat.send_message("👇 Yangi egalik qiluvchi foydalanuvchi yoki admin ID raqamini raqamlarda kiriting", reply_markup=cancel_keyboard()))
            return
        ...
        if data.startswith("delpromo:"):
            ...
            promo_list = await get_all_promocodes()
            await api_call(lambda: query.message.edit_text("🎟:", reply_markup=promo_list_keyboard(promo_list)), action_desc="dp_e")
            return
    except Exception:
...
async def handle_admin_text_state(update, context, user_id, text):
    ...
    if state not in ("add_channel", "set_price", "broadcast", "add_admin",
                     ...
                     "create_promo", "set_payment_channel", "transfer_ownership"):
        return False
    if text == CANCEL_TEXT:
        context.user_data["state"] = None
    ...
    if state == "transfer_ownership":
        ...
        try:
            ...
            await api_call(lambda: update.message.reply_text("❌ Noto'g'ri ID format. ID raqamlardan iborat bo'lishi kerak.", reply_markup=main_keyboard(user_id)))
            return True
        
        await add_admin_db(new_owner_id)
...
async def handle_buttons(update, context):
    try:
        ...
        if _bot_maintenance and not is_admin:
        ...
        if await handle_withdraw_state(update, context, user_id, text):
            return
        if await handle_withdraw_amount_state(update, context, user_id, text):
            return
...
async def withdrawal_reminder_loop(app):
    while True:
        try:
            due = await get_due_withdraw_reminders()
            for wid, uid, amount in due:
                res = await api_call(lambda u=uid, a=amount: app.bot.send_message(
                    chat_id=u,
                    text=(f"💰 *Pulingiz tushdimi?*\n{DIVIDER}\n"
                          f"*{a:,.0f} so'm* miqdoridagi so'rovingiz tasdiqlangandi.\n"
                          f"Agar hali tushmagan bo'lsa, qo'llab-quvvatlash markaziga murojaat qiling."),
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=withdraw_reminder_keyboard()), action_desc="wd_remind")
                if res:
                    await mark_withdraw_reminder_sent(wid)
        except Exception:
            logger.exception("eslatma tsikli xato")
...
async def main_async():
    ...
    app.add_handler(CallbackQueryHandler(start_support_callback, pattern="^start_support$"))
    app.add_handler(CallbackQueryHandler(withdraw_type_callback, pattern="^(wd_type:|wd_cancel$)"))
    app.add_handler(CallbackQueryHandler(withdraw_admin_decision_callback, pattern="^(wdok:|wdno:|wdpaid:)"))
    ...
    global BOT_USERNAME
    me = await api_call(lambda: app.bot.get_me(), action_desc="startup_me", swallow=True, default=None)
    if me:
        BOT_USERNAME = me.username
    reminder_task = asyncio.create_task(withdrawal_reminder_loop(app))
